...
func Login(ctx context.Context, opts LoginOptions, appOpts app.Options) error {
...
func completeAuthLogin(
	...
	if err != nil {
		return fmt.Errorf("write auth output: %w", err)
	}
...
func Status(appOpts app.Options) error {
	...
	err = output.WriteOutput(appOpts, status.toLines())
	if err != nil {
		return fmt.Errorf("write status output: %w", err)
	}
...
func requireClientCredentials(config authClientConfig, missingErr error) error {
	if config.ClientID == emptyString || config.ClientSecret == emptyString {
		return app.NewExitError(app.ExitCodeUsage, missingErr)
	}
...
func confirmLogout(opts LogoutOptions, appOpts app.Options) (bool, error) {
	...
	ok, err := confirm("Delete stored tokens? [y/N]: ", appOpts)
	if err != nil {
...
func classifyTokenError(err error) error {
	...
	if errors.As(err, &apiErr) {
		return app.NewExitError(app.ExitCodeAPI, err)
	}

	return app.NewExitError(app.ExitCodeFailure, err)
}
